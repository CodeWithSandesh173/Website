// ========================================
// REVIEWS.JS - Reviews with Edit Feature
// ========================================
...
// UPDATE REVIEW FORM
// ========================================
function updateReviewForm(user) {
...
function createReviewCard(review) {
    ...
    // User can edit their own review, owner can delete any
    const isOwnReview = currentUser && review.userId === currentUser.uid;
    const showEdit = isOwnReview;
    const showDelete = isOwnerUser || isOwnReview;
    ...
    <p class="review-text" id="text-${review.id}">${escapeHtml(review.text)}</p>
    ${(showEdit || showDelete) ? `
      <div class="review-controls">
        ${showEdit ? `
          <button onclick="editReview('${review.id}')" class="control-btn edit">
            <i class="fas fa-edit"></i> Edit
          </button>
        ` : ''}
        ${showDelete ? `
          <button onclick="deleteReview('${review.id}')" class="control-btn delete">
...
function editReview(reviewId) {
    ...
    const textInput = document.getElementById('reviewText');
    const submitBtn = document.getElementById('submitReviewBtn');
    const formTitle = document.querySelector('#reviewFormSection h3') || document.querySelector('.review-form-card h3');
    ...
    showToast('Editing your review...', 'success');
}
...
async function deleteReview(reviewId) {
    ...
    // Owner can delete any, users can delete their own
    const canDelete = isOwnerUser || (currentUser && review.userId === currentUser.uid);
    ...
    try {
        ...
        // If we were editing this review, cancel
